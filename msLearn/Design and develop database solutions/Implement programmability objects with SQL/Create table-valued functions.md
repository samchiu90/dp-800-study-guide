...
The [`CROSS APPLY`](https://learn.microsoft.com/en-us/sql/t-sql/queries/from-transact-sql#using-apply) operator calls the function for each row from the Customers table, though in this example, the function parameters are constants. When you pass column values as parameters, `CROSS APPLY` becomes particularly useful:
