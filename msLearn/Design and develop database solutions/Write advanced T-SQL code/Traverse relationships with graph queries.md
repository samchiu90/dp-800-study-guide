...
![Diagram showing a graph data model with Person and Product nodes connected by KNOWS and PURCHASED edges.](https://learn.microsoft.com/en-us/training/wwl-data-ai/write-advanced-sql-code/media/graph-model.png)
...
The following example finds friends of friends by chaining two _KNOWS_ relationships. The pattern `Person1-(k1)->Person2-(k2)->Person3` starts at Person1, follows one _KNOWS_ edge to Person2, then follows another _KNOWS_ edge to reach Person3:
...
You can also combine different relationship types in a single traversal. The following example crosses from _KNOWS_ to _PURCHASED_ edges to find what products were bought by people that a given person knows:
...
For more information about graph features, see [Graph processing with SQL Server](https://learn.microsoft.com/en-us/sql/relational-databases/graphs/sql-graph-overview) and [`MATCH` (Transact-SQL)](https://learn.microsoft.com/en-us/sql/t-sql/queries/match-sql-graph).
