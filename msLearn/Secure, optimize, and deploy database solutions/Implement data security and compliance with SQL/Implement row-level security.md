...
![Diagram showing Row-Level Security in a multitenant database: three users query the same Customers table, filtered by a Security Policy component.](https://learn.microsoft.com/en-us/training/wwl-data-ai/implement-data-security-compliance/media/row-level-security.png)
...
A [**security policy**](https://learn.microsoft.com/en-us/sql/t-sql/statements/create-security-policy-transact-sql) binds your predicate functions to tables and specifies the type of filtering. You can create **filter predicates** that silently exclude unauthorized rows from query results, or **block predicates** that prevent unauthorized insert, update, and delete operations.
