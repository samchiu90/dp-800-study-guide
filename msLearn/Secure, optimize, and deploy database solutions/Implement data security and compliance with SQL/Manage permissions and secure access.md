...
SQL Server uses a [hierarchical permission model](https://learn.microsoft.com/en-us/sql/relational-databases/security/permissions-hierarchy-database-engine) where permissions granted at higher levels flow down to lower levels. Think of it as a cascade: server → database → schema → individual objects.
...
Three statements control permissions: [`GRANT`](https://learn.microsoft.com/en-us/sql/t-sql/statements/grant-transact-sql) gives users specific permissions, [`REVOKE`](https://learn.microsoft.com/en-us/sql/t-sql/statements/revoke-transact-sql) removes previously granted permissions, and [`DENY`](https://learn.microsoft.com/en-us/sql/t-sql/statements/deny-transact-sql) explicitly blocks permissions—overriding any grants.
...
Instead of granting permissions directly to users, create [database roles](https://learn.microsoft.com/en-us/sql/relational-databases/security/authentication-access/database-level-roles) that represent job functions. Assign permissions to roles, then add users to the appropriate roles:
...
[Schemas](https://learn.microsoft.com/en-us/sql/relational-databases/security/authentication-access/create-a-database-schema) provide a natural way to organize objects and manage permissions. Group related objects together, then grant permissions at the schema level:
...
Modern applications should use passwordless authentication whenever possible. [Microsoft Entra authentication](https://learn.microsoft.com/en-us/azure/azure-sql/database/authentication-aad-overview) eliminates the risks associated with password management, including credential theft and the operational burden of rotating secrets.
...
[Managed identities](https://learn.microsoft.com/en-us/azure/active-directory/managed-identities-azure-resources/overview) are the most secure option for Azure-hosted applications. Azure handles the identity lifecycle automatically, and there are no credentials that could be leaked or stolen.
