...
![Diagram that shows the four Dynamic Data Masking functions with before and after comparisons for Default, Email, Random, and Partial.](https://learn.microsoft.com/en-us/training/wwl-data-ai/implement-data-security-compliance/media/masking.png)

The **default** function replaces the entire value with a fixed string. For strings, you see _"XXXX"_ (or fewer X characters for shorter columns). Numbers display as zero, and dates show _"01-01-1900"_. Use this when you want complete obfuscation.

The **email** function reveals just the first character of an email address, replaces the middle with "XXX", and preserves the domain suffix. So _"john.smith@contoso.com"_ appears as _"jXXX@XXXX.com"_. The data still looks like a valid email, but the actual address stays hidden.
...
The **partial** function gives you precise control. You specify how many characters to show at the start, what padding characters to use in the middle, and how many to show at the end. For example, a phone number might appear as _"206-XXX-XX89"_.
...
By default, users see masked data unless they have elevated permissions. The [`UNMASK` permission](https://learn.microsoft.com/en-us/sql/relational-databases/security/dynamic-data-masking#permissions) controls who sees the real values behind the masks.
