...
![Screenshot of the Regressed Queries report in SSMS, showing a multi-plan query, a plan comparison chart, and buttons to force or unforce a plan.](https://learn.microsoft.com/en-us/training/wwl-data-ai/optimize-database-performance/media/regressed-queries-view.png)
...
![Screenshot of the Query Store folder in SSMS Object Explorer.](https://learn.microsoft.com/en-us/training/wwl-data-ai/optimize-database-performance/media/query-store-object-explorer.png)
...
![Screenshot of Query Performance Insight in the Azure portal.](https://learn.microsoft.com/en-us/training/wwl-data-ai/optimize-database-performance/media/query-performance-insight.png)
...
>[!Note]
>
>Query Performance Insight is limited to displaying the top 5 to 20 queries. If your workload includes many smaller queries that collectively consume significant resources, they don't appear in QPI. For deeper analysis, query the Query Store catalog views directly or use SSMS. Additionally, also note that Query Performance Insight requires Query Store to be active. If Query Store runs out of space and enters read-only mode, QPI can't display new data. Increase the Query Store maximum size or clear old data to restore normal operation.
